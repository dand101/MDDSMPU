...
RUN_DIR = HERE / "runs" / "fusion_final_dataset_2_clean_text"
...
def plot_learning_curves(history: List[Dict], out_dir: Path):
    ...
    plt.xlabel("epoch"); plt.ylabel("macro-F1"); plt.title("Macro-F1"); plt.legend()
...
train_ds = FusionDataset(train_rows, tokenizer, MAX_LEN, train_tf)
val_ds = FusionDataset(val_rows, tokenizer, MAX_LEN, eval_tf)
test_ds = FusionDataset(test_rows, tokenizer, MAX_LEN, eval_tf)
...
def build_optimizer(stage: str) -> torch.optim.Optimizer:
    params = []
...
def train_one_epoch(loader, optimizer) -> Dict[str, float]:
    model.train()
    total_loss = 0.0
    total = 0
    correct = 0
...
print(f"[INFO] schedule: freeze_img_epochs={WARMUP_EPOCHS_FREEZE_IMG} then unfreeze ResNet layer4 at epoch {UNFREEZE_IMG_LAYER4_AT}")

set_requires_grad(model.img_enc, False)
optimizer = build_optimizer(stage="freeze_img")

for epoch in range(1, EPOCHS + 1):
    if epoch == UNFREEZE_IMG_LAYER4_AT:
        unfreeze_resnet_layer4_only(model.img_enc)
        optimizer = build_optimizer(stage="unfreeze_img")
    ...
    tr = train_one_epoch(train_loader, optimizer)
