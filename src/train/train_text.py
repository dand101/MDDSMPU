...
RUN_DIR = HERE / "runs" / "text_only_clean_final_dataset_2"
...
def calibration_bins(y_true: List[int], y_prob: List[float], n_bins: int = 10):
    ...
    return bin_centers, frac_pos, avg_conf, counts
...
def plot_calibration(y_true: List[int], y_prob_pos: List[float], out_path: Path, n_bins: int = 10):
    centers, frac_pos, avg_conf, counts = calibration_bins(y_true, y_prob_pos, n_bins=n_bins)
...
train_ds = TextDataset(train_rows, tokenizer, MAX_LEN)
val_ds = TextDataset(val_rows, tokenizer, MAX_LEN)
test_ds = TextDataset(test_rows, tokenizer, MAX_LEN)

train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS)
val_loader = DataLoader(val_ds, batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS)
test_loader = DataLoader(test_ds, batch_size=BATCH_SIZE, shuffle=False, num_workers=NUM_WORKERS)

model = TextClassifier(MODEL_NAME).to(DEVICE)
...
def predict_probs(loader) -> Tuple[List[int], List[int], List[float]]:
    """
    Returns y_true, y_pred, y_prob_pos (probability of class 1).
    """
    model.eval()
...
print(f"[INFO] device={DEVICE} | train={len(train_ds)} val={len(val_ds)} test={len(test_ds)} | model={MODEL_NAME}")
